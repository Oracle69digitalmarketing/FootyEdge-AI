...
async def list_teams():
    try:
        ...
        logger.error(f"Error fetching teams from DB: {e}")

    # Fallback to defaults with natural names
    return [
...
async def sync_teams():
    ...
    keywords = ["Man City", "Liverpool", "Arsenal", "Real Madrid", "Barca", "Bayern", "Inter Milan", "PSG", "Chelsea", "Man United", "Spurs", "Dortmund", "Atletico", "Juventus", "AC Milan", "Napoli", "Nigeria", "England", "France", "Brazil", "Argentina", "Germany", "Spain", "Portugal"]
    synced = []
...
if os.path.exists("dist"):
    ...
    async def serve_spa(path: str):
        # If it's an API call that got here, it means it wasn't matched by router
        if path.startswith("api"):
            return JSONResponse(status_code=404, content={"message": f"API endpoint /{path} not found"})

        # Check if requested path is a file in dist
