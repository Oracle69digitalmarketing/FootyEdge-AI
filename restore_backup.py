...
def restore_database_snapshot(filename: str):
    ...
    try:
        ...
        # NOTE: Using 'backup_data' directly as it contains the table data, 
        # based on the backup_manager.py implementation which stores dict{table: data}
        tables = backup_data
        
        logger.warning("⚠️ CRITICAL OPERATION: Wiping active production tables...")
        
        # 2. Delete tables in strict REVERSE hierarchical order to avoid breaking foreign key constraints
        # Order: Child items with foreign keys are wiped first
        ...
        # Parent tables must be seeded before child tables can reference their IDs
        ...
        logger.info(f"🎉 SYSTEM SUCCESS: Database rolled back.")
        return True
...
if __name__ == "__main__":
    ...
    if args.list or not args.file:
        ...
        # Prompt human verification step before executing a data wipe execution path
