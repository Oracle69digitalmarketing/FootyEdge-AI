...
CURRENT_SEASON = "2425" # FBref uses 2425 format for current
...
def generate_deterministic_id(name: str) -> int:
    ...
    # Use first 12 hex chars and convert to int for a large but deterministic ID
        ...
        try:
            ...
            for team_name in unique_teams:
                ...
                supabase.table("teams").upsert({
                    "id": str(generate_deterministic_id(team_name)), "name": team_name, "league_name": league,
                    "attack_strength": ts["attack"], "defense_strength": ts["defense"], "updated_at": datetime.now(timezone.utc).isoformat()
                }).execute()

            # Sync Matches & Process Calculations Loop
            for _, row in schedule.iterrows():
                h_id = str(generate_deterministic_id(row['home_team']))
                a_id = str(generate_deterministic_id(row['away_team']))
            ...
            logger.error(f"⚠️ Loop Exception caught on league context {league}: {str(e)}")
            continue
